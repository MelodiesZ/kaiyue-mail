...
Windows 应用包还会把同一份公开根证书和安装脚本放在 `resources` 目录。更新器先按正常流程验证 Authenticode；仅当签名状态为 `UnknownError` 或 `NotTrusted`、且签名证书主题仍匹配凯越公司发布者时，才会调用该脚本，把 SHA-256 固定的根证书安装到当前用户证书库并重新验签一次。证书被替换、发布者不匹配、安装失败或重试后仍不可信时，更新必须继续中止。固定文件名和 SHA-256 只维护在 `app/internal-trust.json`，Windows 应用打包、NSIS 构建和运行时更新器共用这一配置。

公开根证书可以进入源码仓库，但根证书私钥和代码签名私钥绝对不得进入仓库。卸载凯越邮箱时默认保留根证书，以免影响同一用户下的重装和版本回退。

### 已安装的旧客户端（一次性过渡）

不含上述更新器信任引导代码和 `resources` 证书文件的旧客户端，仍会在启动新安装包之前直接拒绝未受信任的 Authenticode 链，无法通过在线更新“自我安装”这个信任。这部分现有终端必须先一次性下发根证书，或者由用户手动下载并运行新安装包。升级到含信任引导能力的版本后，后续安装和更新都会自动维护该信任。
