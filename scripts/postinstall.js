...
const kaiyueConfig = require('../app/kaiyue-config.json');
...
function getMailsyncURL(callback) {
  ...
  let hash = kaiyueConfig.upstream.mailsyncRevision;
  try {
    const out = execSync('git submodule status ./mailsync', {
      cwd: path.resolve(__dirname, '..'),
      stdio: ['ignore', 'pipe', 'ignore'],
    });
    const match = /^[ +-]?([A-Fa-f0-9]{8,40})/.exec(out.toString().trim());
    if (match) {
      hash = match[1];
    }
  } catch (err) {
    console.log(`\n-- Using the pinned Kaiyue Mail mailsync revision ${hash} --`);
  }
  callback(
