...
import { buildKaiyueNotificationUrl } from './windows-notification-utils';
...
class NativeNotifications {
  ...
  private buildWindowsToastXml(options: {
    ...
    const actionsContent = (options.actions || [])
      .map((action, i) => {
        const actionUrl = buildKaiyueNotificationUrl('notification-action', {
          ...context,
          actionIndex: i,
        });
    ...
    const clickUrl = buildKaiyueNotificationUrl('notification-click', context);
