...
// Kaiyue Mail is distributed primarily to the company's Chinese-speaking
// workforce. Users can still choose another language explicitly in Settings.
const automaticLocale = process.env.KAIYUE_DEFAULT_LOCALE || 'zh-CN';
...
function applyKaiyueBrand(text: string) {
  const brandName =
    locale && locale.startsWith('zh') ? KaiyueConfig.brand.nameChinese : KaiyueConfig.brand.name;
  return text.replace(/Mailspring/g, brandName);
}
...
export function localized(en, ...subs): any {
  let i = 0;
  ...
  return applyKaiyueBrand(translated);
}

export function localizedReactFragment(en, ...subs): any {
  let translated = applyKaiyueBrand(localizations[en] || en);
  if (!subs.length) {
    return translated;
  }
