...
class KeyManager {
  ...
  _reportFatalError(err: Error) {
    const hasHelpUrl = !!KaiyueConfig.services.helpUrl;
    const clickedButton = require('@electron/remote').dialog.showMessageBoxSync({
      type: 'error',
      buttons: hasHelpUrl
        ? [localized('Kaiyue Mail Help'), localized('Quit')]
        : [localized('Quit')],
      message:
        err.message ||
        localized(
          `Kaiyue Mail could not store your password securely. Contact your administrator.`
        ),
    });

    if (hasHelpUrl && clickedButton === 0) {
