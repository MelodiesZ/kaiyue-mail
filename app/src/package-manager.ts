...
export function isKaiyueDisabledPackage(packageName: string) {
  return KaiyueConfig.features.disabledInternalPackages.includes(packageName);
}
...
export default class PackageManager {
  ...
  activatePackage(pkg: Package) {
    ...
    // These built-in packages depend on upstream cloud services. Enforce the
    // distribution policy here so an old user config or manual preference
    // change cannot silently reactivate them.
    if (isKaiyueDisabledPackage(pkg.name)) {
      return;
    }
    ...
    if (!pkg.isEngineSet()) {
      ...
      console.error(
        localized(
          `This plugin or theme %@ does not list "mailspring" in its package.json "engines" field. Ask the developer to test it with Kaiyue Mail before enabling it.`,
          pkg.name
        )
      );
      return;
  ...
  installPackageFromPath(packagePath: string, callback) {
    ...
    if (!json.engines || !json.engines.mailspring) {
      return callback(
        new Error(
          localized(
            `The plugin or theme you selected has not been upgraded to support Kaiyue Mail. If you're the developer, update the package.json engines field to include "mailspring".`
          )
        )
      );
    }
