  ...
  */
  ...
  _onIFrameClick = (e: MouseEvent) => {
    ...
    if (target) {
      ...
      // The upstream client used identity-service SSO for billing links. Kaiyue
      // Mail disables that service, so ordinary links must continue through the
      // normal safe external-link path without resolving the disabled endpoint.
      const identityRoot = KaiyueConfig.services.officialIdentityApiEnabled
        ? rootURLForServer('identity')
        : null;
      if (identityRoot && rawHref.startsWith(identityRoot)) {
        const path = rawHref.split(identityRoot).pop();
