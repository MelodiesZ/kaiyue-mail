...
export function rootURLForServer(server: 'identity') {
  if (!KaiyueConfig.services.officialIdentityApiEnabled) {
    throw new Error('The official Mailspring identity service is disabled in Kaiyue Mail.');
  }

  if (KaiyueConfig.services.identityApiBaseUrl) {
    return KaiyueConfig.services.identityApiBaseUrl.replace(/\/$/, '');
  }
