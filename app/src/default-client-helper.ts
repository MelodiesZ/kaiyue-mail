      ...
      async (err, didMakeDefault) => {
        ...
        if (!didMakeDefault) {
          const { response } = await require('@electron/remote').dialog.showMessageBox({
            ...
            defaultId: 0,
            message: localized(
              `Visit Windows Settings to finish making ${KaiyueConfig.brand.name} your mail client`
            ),
            detail: localized(
              `Click 'Open Settings' to open Windows Settings where you can set ${KaiyueConfig.brand.name} as your default email app.`
            ),
          });
          if (response === 0) {
...
export class DefaultClientHelperMac implements DCH {
  ...
  resetURLScheme(scheme: string, callback = (error?: Error) => {}) {
    ...
    return callback(
      success
        ? null
        : new Error(`Failed to remove ${KaiyueConfig.brand.name} as default handler for ${scheme}`)
    );
  }
  ...
  registerForURLScheme(scheme: string, callback = (error?: Error) => {}) {
    ...
    return callback(
      success
        ? null
        : new Error(`Failed to set ${KaiyueConfig.brand.name} as default handler for ${scheme}`)
    );
  }
}
