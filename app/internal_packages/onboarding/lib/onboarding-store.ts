...
class OnboardingStore extends MailspringStore {
  ...
  constructor() {
    ...
    if (existingAccountJSON) {
      ...
      if (this._account.provider === 'gmail') {
        ...
        this._account.provider === 'imap' &&
        this._account.settings.imap_host === KaiyueConfig.mail.host
      ) {
        this._pageStack = ['account-settings-kaiyue'];
      ...
      // A restored profile may contain accounts but no upstream identity. Never
      // route it to the disabled Mailspring authentication page.
      this._pageStack = ['account-choose'];
    } else {
      // Kaiyue Mail starts with the enterprise account form. A Mailspring
      // identity is optional and never blocks local IMAP / SMTP setup.
      this._account = buildKaiyueAccount('', '');
      this._pageStack = ['account-settings-kaiyue'];
    }
  }
  ...
  _onChooseAccountProvider = (provider: string) => {
    if (provider === 'kaiyue') {
      ...
      this._onMoveToPage('account-settings-kaiyue');
      return;
    }
    const nextPage =
      provider === 'gmail'
