import { extractOAuthCodeFromUrl, OAUTH_COMPLETION_HTML } from '../lib/oauth-signin-page';

describe('OAuth completion page', () => {
  it('is local, branded, and contains no upstream identity URL', () => {
    expect(OAUTH_COMPLETION_HTML).toContain('凯越邮箱');
    expect(OAUTH_COMPLETION_HTML).not.toContain('getmailspring.com');
    expect(OAUTH_COMPLETION_HTML).not.toContain('http://');
    expect(OAUTH_COMPLETION_HTML).not.toContain('https://');
  });
});
